...
<div class="space-y-8" x-data="{ 
    marketers: {{ Js::from($marketers) }},
    selectedMarketerId: '{{ old('marketer_id', '') }}',
    projectValue: '{{ old('project_value', '') }}', 
    feePercentage: '{{ old('fee_percentage', '') }}',
    status: '{{ old('status', 'Pending') }}',
    onMarketerChange() {
        let found = this.marketers.find(m => m.id == this.selectedMarketerId);
        if (found) {
            this.feePercentage = found.default_fee_percentage;
        }
    },
    get calculatedFee() {
    ...
    <!-- Top Section: 3 Metric Cards & Master Data Quick Link -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm relative overflow-hidden flex items-center justify-between">
            <div class="space-y-1">
                ...
                <p class="text-xs text-slate-400">Komisi marketing yang telah lunas</p>
            </div>
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm relative overflow-hidden flex items-center justify-between">
            <div class="space-y-1">
                <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Top Marketer / Channel</span>
        ...
        <div class="lg:col-span-1 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm h-fit">
            <div class="flex items-center justify-between mb-5 pb-3 border-b border-slate-100">
                <div>
                    ...
                    <p class="text-[11px] text-slate-400 mt-0.5">Input data nilai proyek dan komisi marketer.</p>
                </div>
                <a href="{{ route('admin_ops.marketers.index') }}" class="inline-flex items-center space-x-1 px-2.5 py-1 text-[10px] font-bold text-brandGreen bg-emerald-50 border border-emerald-200 rounded-lg hover:bg-brandGreen hover:text-white transition duration-200">
            ...
            <form method="POST" action="{{ route('admin_ops.marketing_fees.store') }}" class="space-y-4">
                ...
                <!-- Nama Marketer / Channel Dropdown -->
                <div>
                    <label for="marketer_id" class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Marketer / Marketing Channel</label>
                    <select id="marketer_id" name="marketer_id" x-model="selectedMarketerId" @change="onMarketerChange()" required
                        class="w-full bg-slate-50 border @error('marketer_id') border-red-500 @else border-slate-200 @enderror rounded-xl px-3.5 py-2.5 text-sm text-slate-700 focus:outline-none focus:border-brandGreen focus:bg-white transition duration-200">
                        <option value="" disabled selected>-- Pilih Marketer / Channel --</option>
                        @foreach($marketers as $marketer)
                            <option value="{{ $marketer->id }}">{{ $marketer->name }} (Def: {{ number_format($marketer->default_fee_percentage, 1) }}%)</option>
                        @endforeach
                    </select>
                    @error('marketer_id')
                ...
                <div>
                    ...
                    <input type="number" id="fee_percentage" name="fee_percentage" x-model="feePercentage" required min="0" max="100" step="0.01"
                    ...
                    <p class="text-[11px] text-slate-400 mt-1.5 font-medium">Terisi otomatis sesuai default marketer, dapat diubah manual jika diperlukan.</p>
                ...
                <div class="p-3.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between">
                    <span class="text-xs font-semibold text-slate-500">Estimasi Fee (Otomatis):</span>
            ...
            <div>
                <div class="flex items-center justify-between mb-6 pb-3 border-b border-slate-100">
                    <div>
                        ...
                        <p class="text-[11px] text-slate-400 mt-0.5">Daftar fee komisi per transaksi beserta status pelunasannya.</p>
                    </div>
                    <span class="px-2.5 py-1 rounded text-[10px] font-bold bg-slate-100 text-slate-600 uppercase tracking-wider">Fee History</span>
                </div>
                    ...
                    <table class="w-full text-left text-xs border-collapse">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-200 text-slate-400 uppercase font-semibold text-[10px] tracking-wider">
                                <th class="py-3.5 px-4">Marketer / Channel</th>
                            ...
                            @forelse($fees as $fee)
                                <tr class="hover:bg-slate-50/50 transition duration-150">
                                    <td class="py-4 px-4 font-bold text-brandNavy">
                                        {{ $fee->marketer->name ?? 'Marketer Dihapus' }}
